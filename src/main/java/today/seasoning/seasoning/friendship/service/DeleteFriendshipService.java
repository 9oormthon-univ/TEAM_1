...
public class DeleteFriendshipService {
    ...
    public void doService(Long deleterId, String toUserAccountId) {
        ...
        Friendship forwardFriendship = friendshipRepository.findByUserIds(deleterId, toUser.getId())
                .orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND, "신청된 내역이 없습니다."));

        Friendship reverseFriendship = friendshipRepository.findByUserIds(toUser.getId(), deleterId)
                .orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND, "신청된 내역이 없습니다."));

        // 삭제자 valid 1, 상대 valid 1
